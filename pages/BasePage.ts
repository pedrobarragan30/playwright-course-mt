import type { Page, Locator } from "@playwright/test"; //"Page" and "Locator" needs to be used for POM
...
export class BasePage {
  ...
  //PB NOTE: <unknown> indicate that this is an "unknown" promise where we do not know what will be returned
  protected step(action: ()=> Promise <unknown>): this{
    this.chain = this.chain.then(() => action());
    return this;
  }
  
  //PB NOTE: The next one is a "generic", those are methods that have no idea what will come in and neither what will come out
  //TResult returns nothing. while TResult2 probably would be never called
